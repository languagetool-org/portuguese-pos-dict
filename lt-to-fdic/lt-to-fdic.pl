...
while ( my $line = <$fh> ) {
    ...
    if ( $line =~ /^([^ ]+) (A.+|N.+) ([^ ]+)$/ ) {
        ...
        if ( $lemma !~ /^$prevLemma$/ ) {    #s'acaba el lema
            ...
            $lemmaFem = "";
            for ( $count = 0 ; $count < 13 ; $count++ ) {  # La posició 9 és per al superlatium
                $forma[$count]   = "";
                $sufixos[$count] = "";
            }
        ...
        if ($forma[0] !~ /^$/ && $postag =~ /AQA/) {
            if ( $postag =~ /MS/ ) {
                $forma[12] = $word; #és superlatiu, ja existeix el lema
            }
        } 
        else {
            if ( $postag =~ /[MC][SN]/ ) {
...
#excepte invariables
            if ( $forma[10] =~ /^.+$/ ) {
                print $ofh " [pl. $forma[2] o $forma[6] o $forma[10]]";
            }
            elsif ( $forma[6] =~ /^.+$/ ) {
...
#excepte invariables
          ...
          )
        {
            ...
            {    #segona forma de femení
                print $ofh " [fem. $forma[5] o $forma[9]]";
            }
            elsif ( $forma[5] =~ /^.+$/ ) 
            {    #segona forma de femení
